...
class LogLike:
    ...
    def _needs_new_mode_selection(self, theta_template):
        """Check if new mode selection is needed based on parameter distance."""
    ...
    def __call__(self, theta_template):
        ...
        # Check if we need new mode selection
        if self._needs_new_mode_selection(theta_template):
            if self.verbose:
                print(f"Generating new modes...")
            ...
            # Cache the new modes
            self._cache_modes(theta_template, selected_modes, selected_labels)
        else:
            if self.verbose:
                print(f"Using cached modes...")
            selected_modes, selected_labels = self._cached_modes, self._cached_labels
